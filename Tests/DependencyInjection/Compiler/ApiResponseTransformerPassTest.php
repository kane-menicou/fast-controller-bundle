...
class ApiResponseTransformerPassTest extends \PHPUnit_Framework_TestCase
{
    /**
     * @test
     */
    ...
    {
        $pass = new ApiResponseTransformerPass;
    ...
    public function itWillAddResponseTransformersToTheChainIfThereDefined()
    {
        ...
        $mockContainer->expects($this->once())->method('findDefinition')
            ->with('kane_menicou_fast_controller.chain.api_response_transformer_chain')
            ->willReturn($mockDefinition = $this->getMock(Definition::class));

        $mockContainer->expects($this->once())->method('findTaggedServiceIds')
            ->with('kane_menicou_fast_controller.api_response_transformer')->willReturn([$transformer]);
